...
export async function generateStaticParams() {
  ...
  try {
    ...
    return product.map(({ slug }: { slug: string }) => ({ slug: slug }));
  } catch (error) {
    console.error(error);
  }
}
...
export async function generateMetadata({ params }: { params: { slug: string }}) {
  try {
    ...
    const product = await getProductBySlug(params.slug, selectedLanguage);
...
export default async function Product({ params }: {params: { slug: string }}) {
  async function getProductByParams() {
    try {
      ...
      const oneProduct = await getProductBySlug(params.slug, selectedLanguage);
