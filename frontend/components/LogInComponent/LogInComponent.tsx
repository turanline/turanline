...
const LogInComponent: FC<{ route: string }> = ({ route }) => {
  const { isAuth, status, isProviderAuth } = useTypedSelector(
    state => state.user
  );
  ...
  useEffect(() => {
    if (status === "fulfilled") {
      if (isAuth || isProviderAuth) {
        push(SHOP_ROUTE);
      }
    }
  }, [isAuth, isProviderAuth, status]);
  ...
  if (status === "pending" || isAuth || isProviderAuth)
